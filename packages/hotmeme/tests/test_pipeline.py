...
@patch("hotmeme.pipeline.fetch_hot_posts.TikHubSource.fetch_platform_result")
def test_fetch_hot_posts_records_platform_errors(mock_fetch) -> None:
    mock_fetch.side_effect = TikHubApiError("TikHub 请求失败: 邮箱未验证")
    client = HotMeme(api_key="test-key", platforms=["xiaohongshu"])
    feed = client.fetch_hot_posts()
    assert feed.items == []
    assert feed.providers_failed == ["tikhub"]
    assert len(feed.fetch_errors) == 1
    assert all("邮箱未验证" in err for err in feed.fetch_errors)
    assert all(err.startswith("[xiaohongshu]") for err in feed.fetch_errors)
