...
- ``output_files``：需要交还给用户的文件路径数组，路径**相对 Harness 工作区根**（如 ``incoming/foo.xlsx``、``out/result.png``、``out/summary.mp3``）；可含图片、音视频等任意已存在文件；无文件则 ``[]``
...
首行须写：``会话当前日期：YYYY-MM-DD（来源：current_time__get_current_time）``（本步已调用该工具时；日期取自工具返回）；首行前勿加 ``---`` 或其它前缀。
其后用简洁中文记录本步**结论要点**（检索时间窗口、事实、条目），供后续步骤使用；条数贴近用户所要的条数，勿堆砌窗口外月份背景。
本步各回合可见回复均用中文；勿写搜前搜后寒暄、「进入下一步」等过程叙述。
本步对搜索类工具**只调用一次**，以其返回为事实依据，勿追加搜索。
同回合**禁止**与 ``current_time__get_current_time`` 一并发起 ``cursor_cli__run_cursor_agent`` 等搜索类工具：须先单独一轮取时，待返回后再搜；并行发起时运行层只执行取时，搜索须下回合再发起。
